...
============================================
��һ���̣��Ȳ�ֲ� �� ���ֹ�� �� ִ�е���
handlebar ÿ ~5s ������Ĭ��Ʒ��=SH000300������=1���ӣ�
"""
import json, os
...
REBALANCE_TIME = "09:45"  # �����ӳٵ�����45���Ӻ󣬱ܿ����̾��Ҳ���

class G:
    stops = {}              # {code: {peak, threshold_pct, trail_profit_pct, qty, name, cost}}
    sold_codes = set()      # �Ѵ���ֹ��Ĵ���
    last_minute = ""
    batch_day = 0           # ��������: 1=D1(��һ) 2=D2(����) 3=D3(����) 0=�ǽ�����
    holdings = {}            # ʵ�ʳֲ� {qmt_code: ���ù���}
    orders = None            # ����ָ�� JSON
    rebalance_done_date = "" # ��������Ƿ���ִ�У������ַ�����
    rebalance_phase = ""     # "" | "submitted" | "verified" �� ��֤״̬��
    expected_holdings = {}   # {code: target_shares}
    pending_orders = {}      # ��ȷ��ί�� {remark: time}  ���ظ��µ� �� ����Ŀ��������֤
    d1_date = ""            # ����D1���ڣ���һ����һ��
    d2_date = ""            # ����D2���ڣ���һ��������
    d3_date = ""            # ����D3���ڣ���һ�����壩
    deals = []               # �ɽ���¼ [{time, code, direction, volume, price, amount, order_id}]
    orders_log = []          # ί�м�¼ [{time, code, direction, volume, filled, price, status, order_id}]
g = G()

def get_script_dir():
    try: return os.path.dirname(os.path.abspath(__file__))
    except: pass
    for d in [r"D:\���ǲ��Խ���ϵͳ\python", os.getcwd()]:
        if os.path.isdir(d): return d
    return os.getcwd()

def load_orders_file():
    """加载指令文件, 优先当前目录, 回退硬编码路径"""
    import os, json
...
#  ������ order_* ϵ�У��Զ���۸񡢷���ȡ����
#  ֹ���� passorder����Ҫ quickTrade=2 ����ִ�У�
# �T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T

def do_sell(C, qmt_code, qty, label):
    """���� qty �ɡ�GWT order_shares �Զ�ȡ���ּۡ�����"""
    avail = g.holdings.get(qmt_code, 0)
    if avail <= 0:
        g.holdings = query_holdings()
        avail = g.holdings.get(qmt_code, 0)
        if avail <= 0:
            print(f"  [SKIP] {qmt_code} �޳ֲ�")
            return
    if qty > avail:
        print(f"  [ADJ] {qmt_code} ָ��{qty}�ɡ�{avail}��")
        qty = avail
    qty = int(qty / 100) * 100
    if qty <= 0: return
...
def do_buy(C, qmt_code, amount_yuan, label):
    """���� amount_yuan Ԫ���� passorder(1101) �ֹ����������order_value ʵ�̲���Ч"""
    tick = C.get_full_tick([qmt_code])
    if not tick or qmt_code not in tick: return
    t = tick[qmt_code]
    ...
    if shares < 100:
        print(f"  [SKIP] {qmt_code} {amount_yuan}Ԫ ����1��(@{price:.3f})")
        return
    if EXECUTE_REAL:
    ...
    for code, info in list(g.stops.items()):
        ...
        try:
            ...
            if reason:
                ...
                try:
                    passorder(24, 1101, account, qmt_code, 11, order_price, info['qty'], 'ֹ��', 2, '', C)
                    print(f"  [DONE] {qmt_code} x{info['qty']} @{order_price:.3f}")
                    g.sold_codes.add(code)
                    g.holdings.pop(qmt_code, None)  # �ӳֲֻ����Ƴ�
...
def run_rebalance(C):
    ...
    if total_value <= 0:
        total_value = 100000
        print(f"  [��ֵ] �ǽ���ʱ�Σ��ù̶����ʲ�{total_value/10000:.0f}��")

    # �T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T
    # Step 1: ����Ŀ�� + ����Ŀ�꣨D3=100%���ֲ�����
    # �T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T
    cum_ratio = sum(BATCH_RATIOS[:g.batch_day])
    print(f"\n{'='*50}")
    print(f"  D{g.batch_day} ���� | ���ʲ���{total_value/10000:.1f}�� | Ŀ��{cum_ratio*100:.0f}%")
    print(f"{'='*50}")
    print(f"\n  [Step1] ����Ŀ�� (�ۼ�{cum_ratio*100:.0f}%) / ����Ŀ�� (100%):")
    target_quantities = {}
    final_quantities = {}  # D3=100%Ŀ�꣬�����ж��Ƿ���������
    ...
    if g.holdings:
        for qc, vol in g.holdings.items():
            code = qc.split(".")[0]
            tag = "Ŀ����" if code in target_weights else "�����"
            print(f"    {code}: {vol}�� ({tag})")
    else:
        print(f"    (�ղ�)")

    # �T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T
    # Step 3: ��������Ŀ����� + Ŀ�곬����֣�������Ŀ��=100%Ϊ��׼��
    # �T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T
    sell_list = []
    for qc, vol in g.holdings.items():
        code = qc.split(".")[0]
    ...
    if sell_list:
        ...
        for qc, code, vol, reason in sell_list:
            try:
                passorder(24, 1101, account, qc, 0, 0, vol, "�¶ȵ���", 2, '', C)
    ...
    for code, target_shares in target_quantities.items():
        ...
        if delta >= 100:
            ...
            print(f"  [Step4] {code} ���{delta}�ɲ���1�֣�����")

    if buy_list:
        print(f"\n  [Step4] ����Ŀ��ETF ({len(buy_list)}��):")
        for qmt_code, code, delta, price in buy_list:
            try:
                passorder(23, 1101, account, qmt_code, 11, price, delta, "�¶ȵ���", 2, '', C)
...
def _verify_rebalance(C, target_weights, today, batch_path, batch_state):
    ...
    for code, expected in g.expected_holdings.items():
        ...
        if diff > 0:
            all_ok = False
            msg = f"ȱ{code}: Ӧ��{expected}�� ʵ��{actual}�� ȱ{diff}��"
            discrepancies.append(msg)
            print(f"  [ȱ] {msg}")
        elif diff < 0:
            all_ok = False
            msg = f"��{code}: Ӧ��{expected}�� ʵ��{actual}�� ��{-diff}��"
            discrepancies.append(msg)
            print(f"  [��] {msg}")
        else:
    ...
    if all_ok:
        print(f"\n  [���] �ֲ���D{g.batch_day}Ŀ��һ�� OK")
    else:
        print(f"\n  [ע��] ����{len(discrepancies)}�����:")
        for d_msg in discrepancies:
            print(f"    - {d_msg}")
        print(f"  ����ԭ��: ί��δ�ɽ�/���ֳɽ�/�ύʧ��")
    ...
    print(f"  [�־û�] rebalance_done_date={today} �� �������ٽ���\n")

# �T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T�T
def init(C):
    print(f"\n{'='*55}")
    print(f"  QMT ͳһ���� | {'ʵ��' if EXECUTE_REAL else 'ģ��'}")
    print(f"{'='*55}")

    # 1. ��ֲ�
    g.holdings = query_holdings()
    if g.holdings:
        print(f"  [�ֲ�] {len(g.holdings)}ֻ")
    ...
    if g.orders:
        ...
        if g.stops:
            try:
                ...
                print(f"  [U] universe�Ѷ���")
            except: pass
    else:
        print(f"  [ָ��] �ޡ�����ֹ����ģʽ")

    # 3. �¶ȵ������ڣ�D1=��һ����һ, D2=��һ������, D3=��һ�����壩
    now = datetime.now()
    ...
    # ==== �ڼ����ֶ������������ڼ���˳��������ͬһ�գ�====
    # ʾ����2026��5�� 5/1�Ͷ��ڡ�D3˳����5/8
    # if cycle == "202605":
    #     g.d3_date = "2026-05-08"
    # =================================================

    print(f"  [������] D1={g.d1_date}(��һ) D2={g.d2_date}(����) D3={g.d3_date}(����)")
            ...
            if po["code"] in code and po["qty"] == vol:
                ...
                if elapsed > 30:
                    try:
                        ...
                        print(f"  [����] {code} x{vol} ��ʱ{elapsed:.0f}s")
                    except:
                        pass
...
def save_trade_report():
    ...
    report = {
        "updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "deals": g.deals[-500:],    # ���500�ʳɽ�
        "orders": g.orders_log[-200:],  # ���200��ί��
    }
    try:
...
def deal_callback(C, dealInfo):
    ...
    try:
        ...
        g.deals.append(d)
        print(f"[�ɽ�] {d['time']} {d['direction']} {code} x{vol} @{price:.3f} ={amount:.2f}")
        # ��Ͷ�ʱ�ע�Ƴ��ѳɽ�ί��
...
def order_callback(C, orderInfo):
    """ί�лر���ί��״̬�仯ʱQMT�Զ�����"""
    try:
        status_map = {0: "δ��", 1: "�ѱ�", 2: "����", 3: "ȫ��", 4: "����", 5: "ȫ��", 6: "�ϵ�"}
        ...
        if st >= 3:
            print(f"[ί��] {o['time']} {o['direction']} {o['code']} {o['volume']}�� {o['status']}")
        save_trade_report()
    except Exception as e:
...
def account_callback(C, accountInfo):
    ...
    try:
        ...
        if status:
            print(f"[�˻�] {datetime.now().strftime('%H:%M:%S')} ״̬: {status}")
    except:
        pass
